...
def actualizar_estado(identificador: str, estado: str, datos_adicionales: dict = None):
    """
    ...
    db.collection('estados').document(identificador).set(datos)
    return datos

def obtener_tareas_activas():
    """
    Obtiene las tareas activas del nodo
    """
    return db.collection('tareas').where('nodo', '==', nombre_nodo).where('estado', '==', 'activo').get()

def obtener_tareas_terminadas():
    """
    Obtiene las tareas terminadas del nodo
    """
    return db.collection('tareas').where('nodo', '==', nombre_nodo).where('estado', '==', 'terminado').get() 