class SnakeGame {
    ...
    update() {
        ...
        if (this.isSpecialMode) {
            ...
            this.segmentHues = this.snake.map((_, index) => (this.colorHue + index * 30) % 360);
            if (Date.now() > this.specialModeEndTime) {
                this.deactivateSpecialMode();
            } else {
                const remainingTime = Math.ceil((this.specialModeEndTime - Date.now()) / 1000);
    ...
    activateSpecialMode() {
        ...
        this.speed = Math.max(30, this.speed - 20); // Aumenta la velocidad
        this.segmentHues = this.snake.map((_, index) => (this.colorHue + index * 30) % 360);
    ...
    deactivateSpecialMode() {
        this.isSpecialMode = false;
        this.speed = 150; // Velocidad normal
    ...
    draw() {
        ...
        if (this.isSpecialMode) {
            ...
            bodyGradient.addColorStop(0, `hsla(${hue1}, 100%, 90%, ${0.98 + this.glowIntensity * 0.02})`);
            bodyGradient.addColorStop(0.17, `hsla(${hue2}, 100%, 95%, ${0.98 + this.glowIntensity * 0.02})`);
            bodyGradient.addColorStop(0.33, `hsla(${hue3}, 100%, 90%, ${0.98 + this.glowIntensity * 0.02})`);
            bodyGradient.addColorStop(0.5, `hsla(${hue4}, 100%, 95%, ${0.98 + this.glowIntensity * 0.02})`);
            bodyGradient.addColorStop(0.67, `hsla(${hue5}, 100%, 90%, ${0.98 + this.glowIntensity * 0.02})`);
            bodyGradient.addColorStop(0.83, `hsla(${hue6}, 100%, 95%, ${0.98 + this.glowIntensity * 0.02})`);
            bodyGradient.addColorStop(1, `hsla(${hue1}, 100%, 90%, ${0.98 + this.glowIntensity * 0.02})`);

            // Añadir efecto de brillo ultra intenso
            this.ctx.shadowColor = `hsla(${hue1}, 100%, 80%, ${0.8 + this.glowIntensity * 0.2})`;
        ...
        if (this.isSpecialMode) {
            ...
            const gradient = this.ctx.createRadialGradient(
                headX + segmentSize/2, headY + segmentSize/2, 0,
                headX + segmentSize/2, headY + segmentSize/2, segmentSize/2
            );
            gradient.addColorStop(0, `hsla(${hue}, 100%, 95%, ${0.99 + this.glowIntensity * 0.01})`);
            gradient.addColorStop(0.25, `hsla(${(hue + 20) % 360}, 100%, 90%, ${0.99 + this.glowIntensity * 0.01})`);
            gradient.addColorStop(0.5, `hsla(${(hue + 40) % 360}, 100%, 85%, ${0.99 + this.glowIntensity * 0.01})`);
            gradient.addColorStop(0.75, `hsla(${(hue + 60) % 360}, 100%, 80%, ${0.99 + this.glowIntensity * 0.01})`);
            gradient.addColorStop(1, `hsla(${(hue + 80) % 360}, 100%, 75%, ${0.99 + this.glowIntensity * 0.01})`);
            this.ctx.fillStyle = gradient;
            this.ctx.shadowColor = `hsla(${hue}, 100%, 80%, ${0.8 + this.glowIntensity * 0.2})`;
        ...
        this.ctx.beginPath();
        this.ctx.arc(eyeX1 - highlightSize/2, eyeY1 - highlightSize/2, highlightSize, 0, Math.PI * 2);
        this.ctx.arc(eyeX2 - highlightSize/2, eyeY2 - highlightSize/2, highlightSize, 0, Math.PI * 2);
        this.ctx.fill();
        ...
        if (this.food.isSpecial) {
            ...
            // Brillo
            const glowGradient = this.ctx.createRadialGradient(0, 0, 0, 0, 0, this.gridSize);
    ...
    drawApple(x, y, size) {
        ...
        this.ctx.quadraticCurveTo(x + size/2, y - size - 5, x + size/2, y - size);
        this.ctx.stroke();

        // Dibujar la hoja
        this.ctx.fillStyle = '#27AE60';
        this.ctx.beginPath();
        this.ctx.ellipse(x + size/2, y - size, size/4, size/2, Math.PI/4, 0, Math.PI * 2);
        this.ctx.fill();
    }
